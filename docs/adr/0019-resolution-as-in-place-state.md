...
2. **Commit-driven resolves silently.** On a positive per-thread judgment the daemon writes the stamp (naming the resolving commit) and resolves the thread. It posts no threaded note and opens no summary review. This supersedes ADR 0017's note-then-resolve (Decision point 3). The resolve is gated on the judgment, not the stamp's wording: a rationale that trips `voice.py` is degraded to a generic stand-in and the resolve still proceeds (#168). Blocking it on the rationale's voice would revive ADR 0017's retired "never resolve without a clean note" coupling, now a priority inversion: the stamp is a silent edit, so a cosmetic lexical rule has no business gating a merge-critical state change.
...
- A stamp rationale that fails `voice.py` no longer leaves the thread open (#168). The resolve is gated on the fix judgment alone; the off-voice rationale degrades to a generic stand-in, so the daemon's PR text stays voice-clean and the merge gate still clears. Leave-open stays scoped to judgment uncertainty (Safety layer 1), never to the stamp's wording.
