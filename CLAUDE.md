...
**Background, optional** (always-on across logout/reboot): `bash bin/install.sh` registers a `KeepAlive` launchd job running the same loop (supervising the process rather than firing a `StartInterval` timer, which stalled silently across sleep/wake, #83); stop with `bash bin/uninstall.sh`. Logs flow to `.daemon.log`; liveness: `echo $(( $(date +%s) - $(cat ~/.local/state/pr-review-agent/daemon.heartbeat) ))s since last cycle`. Caveat (ADR 0011): the launchd job is invisible and bound to this checkout's working tree, so keep that checkout on `main` — switching its branch silently breaks the running daemon. If you also develop here, run the dogfood daemon in the foreground or from a separate clone.
