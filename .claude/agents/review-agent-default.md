...
Write sentences that are **clear, concise, and elegant**. Specifically:

- Avoid the em dash (`—`). It is an AI tell. Use periods, commas, parentheses, or a new sentence instead.
- Prefer short declarative sentences. If a sentence runs past one line, consider splitting it.
- Cut filler ("just", "actually", "basically", "I think", "it seems like").
- Lead with the observation, then the consequence. Save the suggestion for the end.
- One idea per finding body. If you have two ideas about the same line, pick the load-bearing one.
