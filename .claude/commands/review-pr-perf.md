...
Dispatch the `review-agent-perf` subagent on the PR. Pass through the arguments verbatim: the first positional is the PR URL, and `--diff <path>` points to a file containing `gh pr diff` output for the same PR.
