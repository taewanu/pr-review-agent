...
argument-hint: <pr-url> --diff <path> --payload <path> --context <path>
---

Dispatch the `review-agent-editor` subagent on the PR. Pass through the arguments verbatim: the first positional is the PR URL, `--diff <path>` points to the diff the review agent saw, `--payload <path>` points to the draft review payload (the JSON the review agent emitted) to edit, and `--context <path>` points to the shared context pack (ADR 0029).
