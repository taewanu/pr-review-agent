...
**Provenance tag**:
The compact `🤖 _pr-review-agent_` appended to every posted artifact that is not a Finding Review body (each Inline comment, each daemon reply, and the Status comment), answering "who wrote this" under the shared solo identity (ADR 0003). The split is by artifact level, not draft-status: the Finding Review body carries the Review footer; everything else carries the Provenance tag. It never encodes draft-status (that is the Review footer's job), so it is byte-identical on pending and posted artifacts. Generalizes the #82 reply fix into a rule. Format pinned by ADR 0010.
...
**Review agent**:
A directly-prompted `claude -p` process that reads a PR and emits structured findings, defined as a file at `.claude/agents/review-agent-*.md`. Two fixed roles run in parallel (ADR 0038), split on contamination isolation: `code` reads the diff, the surrounding code, and the repo's conventions, quarantined from every author claim; `intent` reads the claims (description, linked issues, commit messages) against the diff, and skips itself on a PR that describes nothing (ADR 0035). There is no role dial; the former lens taxonomy and its `REVIEW_LENSES`/`REVIEW_MODE` dials are gone. Review agents are stateless and do not post; the daemon's posting step handles the GitHub side. Use the hyphenated form "review-agent" when the compound serves as a single token (identifiers, file names).
...
**Reply ack**:
The threaded reply the daemon posts to answer an Operator's `fix_claim` or `question` reply, posted **detached**: one `/replies` POST per ack, opening no review wrapper (#159, ADR 0019). Each ack attaches to its own Reply thread (the `PRRT_` id) and carries the Verdict-leading body, the Provenance tag, and the per-reply Reply sentinel. A reply-tick with N body-bearing replies posts N detached acks and so sends N notifications; the burst is accepted as rare (a first-review reply pass) against the cost it replaces, a standing empty review object on every reply-tick. On a resolving Verdict (`confirmed` / `withdrawn`) the daemon also writes the Resolution stamp and resolves the thread; `pushback` / `stands` leave the Finding live. An `acknowledgment` carries no body, so it is a reaction-only ack and posts no Reply ack (see Ack reaction).
_Avoid_: Reply review / disposition summary / reply-review marker (the retired #38 batching wrapper, its one-line body, and the wrapper's hidden marker; #159 detaches the ack, so no wrapper, rollup, or marker remains), batch comment (the ack is no longer a GitHub review object), Resolution stamp (the in-place state edit, not this threaded human-facing answer)
...
- **Commit-driven** (#159, supersedes ADR 0017's `_Fixed:_` note): on a new HEAD SHA the daemon finds a Finding's flagged defect gone at HEAD and resolves with **no Operator reply**. It records the close as a **Resolution stamp** edited into the Finding's own Inline comment (naming the resolving commit), not a separate threaded note. Safe-biased: it resolves only on a positive per-thread judgment, else leaves the thread open.
...
_Avoid_: addressed / done (the Reply sentinel already overloads "addressed"; resolution is strictly the GitHub open→resolved transition), close (GitHub "close" is a PR/issue action, distinct from resolving a thread), `_Fixed:_` as a Verdict (the four Verdicts answer an Operator reply; commit-driven resolution writes a Resolution stamp, not a Verdict)

**Resolution stamp**:
A one-line status edited in place into a resolved Finding's Inline comment, recording why it closed (the resolving commit for a commit-driven resolution; the Verdict for a reply-driven one). An *edit* of the existing comment, never a new threaded note, so it is silent (no notification) and single-slotted: two resolution signals on one thread converge on the same line instead of racing two notes (the #159 fix). It is the visible, reversible audit trace ADR 0017 wanted, without the `_Fixed:_` threaded note's noise. The thread-level twin of the Status comment (PR-level state, edited in place).
_Avoid_: `_Fixed:_` note (the superseded threaded form, #159), Reply ack (a separate threaded reply that answers an Operator's human message, not an in-place state edit)
