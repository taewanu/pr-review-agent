...
**Review agent**:
A Claude Code subagent that reads a PR's diff and emits structured findings. Defined as a file at `.claude/agents/review-agent-*.md`. Six run in parallel as independent lenses (ADR 0023, ADR 0035): `default`, `data-flow`, `security`, `perf`, `tests`, and `intent`, narrowed per operator by `REVIEW_LENSES` (ADR 0034), which defaults to `default data-flow intent`. Five read only the diff; `intent` also reads what the change said it would do. They are not peers: ADR 0037 sorts them into the base sweep (`default`), a data-flow specialist that catches cross-component bugs the base misses (`data-flow`), a structural addition that reads an input the base cannot (`intent`), and opt-in domain depth (`perf`/`security`/`tests`). Review agents are stateless and do not post; the daemon's posting step handles the GitHub side. Use the hyphenated form "review-agent" when the compound serves as a single token (identifiers, file names).
